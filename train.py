...
g_loss = tf.reduce_mean(
    tf.nn.sigmoid_cross_entropy_with_logits(logits=d_fake, labels=tf.ones_like(d_fake))) * 0.1 + tf.reduce_mean(
    tf.abs(tf.subtract(g, highres)))
...
for epoch in range(epochs):
    ...
    for iter in range(iters):
        ...
        loss_g_this_iter, loss_d_this_iter, _ = sess.run([g_loss, d_loss, g_optimizer],
                                                         {lowres: Z, highres: X, isTraining: True})
        sess.run([d_loss, d_optimizer], {highres: X, lowres: Z, isTraining: True})
        if iter % 25 == 0:
            print('Epoch: %d, Passed iterations=%d/%d, Generative cost=%.9f, Discriminative cost=%.9f' % (
            epoch, iter * batch_size, iters * batch_size, loss_g_this_iter, loss_d_this_iter))
